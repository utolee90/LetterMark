...
$wgExtensionCredits['parserhook'][] = array(
	...
	'description' => 'PHP나무마크 미디어위키의 변형인 LetterMark를 적용합니다. (Enable to use LetterMark, a variation of PHP Namumark in MediaWiki )',
	...
	'url' => 'https://github.com/utolee90/LetterMark/tree/master',
   
	'license-name' => "AGPL-3.0",   // Short name of the license, links LICENSE or COPYING file if existing - string, added in 1.23.0
  
);

$wgHooks['ParserBeforeStrip'][] = 'LetterMark';
$wgHooks['InternalParseBeforeLinks'][] = 'LetterMarkHTML';
$wgHooks['ParserBeforeTidy'][] = 'LetterMarkHTML2';
$wgHooks['ParserAfterTidy'][] = 'LetterMarkExtraHTML'; 

require_once('php-lettermark.php');
require_once("LetterMarkExtra.php");
require_once("php-lettermark.class1.php");
require_once("php-lettermark.class2.php");
require_once("php-lettermark.class3.php");


function LetterMark(&$parser, &$text, &$strip_state) {
...
function LetterMarkHTML( Parser &$parser, &$text ) {
...
function LetterMarkHTML2( &$parser, &$text ) {
	$title = $parser->getTitle();
	if (!preg_match('/^특수:/', $title) && !preg_match("/&action=history/", $_SERVER["REQUEST_URI"]) && !preg_match('/^사용자:.*\.(css|js)$/', $title)) {
...
function LetterMarkExtraHTML ( &$parser, &$text ) {
	$title = $parser->getTitle(); // 문서의 제목을 title로 변수화한다.
