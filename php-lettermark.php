...
class LetterMark {
    ...
    function __construct($wtext, $title) { # Ordered Line Parser
...
*/
    protected function mediawikiProcessor($text, $type) { // template parser - keep mediawiki 
        if($type == '{{')
            return '{{'.$text.'}}';
    }
	...
	protected function formatParser($line) { // prevent parser error from opened
	...
	protected function bracketParser($text, &$now, $bracket) { // BracketParser []
		$len = strlen($text);
		$cnt = 0;
		$done = false;
    ...
    protected function textProcessor($otext, $type) {
        ...
        switch($type) {
            ...
            case '__':
                ...
                if(in_array($text, $mw_magic_words) || preg_match('/^.*?(\.jpeg|\.jpg|\.png|\.gif)/', $text)) # &#48120;&#46356;&#50612;&#50948;&#53412; &#53412;&#50892;&#46300; &#50500;&#45776; &#46412;&#47196; &#54869;&#51109;
                    return $type.$text.$type;
                else
                    return '<u>'.$text.'</u>';
            case '^^':
			...
			case '$$': #Replace Math Tag
                if(!self::startsWith($text, ' ') && !self::startsWith($text, 'item-') && !self::endsWith($text, 'UNIQ') && !self::startsWith($text, 'QINU') && !preg_match('/^.*?-.*-QINU/', $text) && !self::startsWith($text, 'h-'))
                    ...
                    return $type.$text.$type; 
			case '<<': #File Parser Tag
                if(preg_match('/^.*?(\.jpeg|\.jpg|\.png|\.gif|\.svg).*/i', $text)){ // File Parser. Format name appears.
					if(self::startsWithi($text, 'File:') || self::startsWithi($text, '파일:') )
						return '[['.$text.']]';
					else {
						$FileName= explode(":", $text, 2); // separate the text by ':'
						if (($FileName[0] === 'F') || ($FileName[0] === '파')) // Specify several prefixes to ignore 
						    return '[[File:'.$Filename[1].']]';
						else
							return '[[File:'.$text.']]'; # Remaining case - File Parser
					}
                }else
                    return $type.$text.'>>'; 
